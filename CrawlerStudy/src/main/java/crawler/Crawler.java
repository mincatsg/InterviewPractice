...
public class Crawler {
    ...
    private HashSet<String> urlBlackList = new HashSet<String>();
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 0; i < projects.size(); i++) {
            try {
                ...
                String repoName = crawler.getRepoName(project.getUrl()); //得到访问githubAPI固定的用户名和仓库名
                String jsonString = crawler.getRepoInfo(repoName);  //得到的html字符串
                // System.out.println(jsonString);
                // 4. 解析每个仓库获取到的 JSON 数据, 得到需要的信息
                crawler.parseRepoInfo(jsonString, project); //把各个仓库需要的数值映射到hashmap中.在设置到project对象中
...
//         使用Jsoup分析一下页面结构,把其中li标签都获取到
//         1.先创建一个Document对象(文档对象, 对应一个html)
//           相当于把一个html字符串转换为Document对象
//           Document相当于一个描述页面的树形结构
//            Document document = Jsoup.parse(html);
//         2.使用getElementByTag 方法来获取到所有的 li标签
//          elements 对象相当于是一个集合类, 包含了许多的Element对象,每个Element就对应到一个"li"标签
